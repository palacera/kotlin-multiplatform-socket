...
class SocketConnectionPoolLogChannel(
    ...
    init {
        launch {
            channelFlow.collect {
                when (it) {
                    is SocketConnectionPoolLog.ConnectingState -> logConnectingState(it)
                    is SocketConnectionPoolLog.ConnectedState -> logConnectedState(it)
                    is SocketConnectionPoolLog.ConnectionAttempt -> logConnectionAttempt(it)
                    is SocketConnectionPoolLog.ConnectionWarning -> logConnectionWarning(it)
                    is SocketConnectionPoolLog.StartPing -> logStartPing(it)
                    is SocketConnectionPoolLog.PingError -> logPingError(it)
                    is SocketConnectionPoolLog.ConfirmingConnectionState -> logConfirmingConnectionState(it)
                    is SocketConnectionPoolLog.AwaitingConfirmationState -> logAwaitingConfirmationState(it)
                    is SocketConnectionPoolLog.ConnectionConfirmedState -> logConnectionConfirmedState(it)
                    is SocketConnectionPoolLog.DisconnectingState -> logDisconnectingState(it)
                    is SocketConnectionPoolLog.DisconnectedState -> logDisconnectedState(it)
                    is SocketConnectionPoolLog.DisposedState -> logDisposedState(it)
                    is SocketConnectionPoolLog.ErrorState -> logErrorState(it)
                    is SocketConnectionPoolLog.Debug -> logDebug(it)
                }
            }
        }
    }
