...
class SocketConnectionLogChannel(
    ...
    init {
        launch {
            channelFlow.collect {
                when (it) {
                    is ConnectionLog.ConnectingState -> logConnectingState(it)
                    is ConnectionLog.ConnectedState -> logConnectedState(it)
                    is ConnectionLog.ConnectionAttempt -> logConnectionAttempt(it)
                    is ConnectionLog.ConnectionWarning -> logConnectionWarning(it)
                    is ConnectionLog.StartPing -> logStartPing(it)
                    is ConnectionLog.PingError -> logPingError(it)
                    is ConnectionLog.ConfirmingConnectionState -> logConfirmingConnectionState(it)
                    is ConnectionLog.AwaitingConfirmationState -> logAwaitingConfirmationState(it)
                    is ConnectionLog.ConnectionConfirmedState -> logConnectionConfirmedState(it)
                    is ConnectionLog.DisconnectingState -> logDisconnectingState(it)
                    is ConnectionLog.DisconnectedState -> logDisconnectedState(it)
                    is ConnectionLog.DisposedState -> logDisposedState(it)
                    is ConnectionLog.ErrorState -> logErrorState(it)
                    is ConnectionLog.Debug -> logDebug(it)
                }
            }
        }
    }
