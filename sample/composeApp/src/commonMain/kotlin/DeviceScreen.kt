...
class DeviceScreen : Screen {
    ...
    override fun Content() {
        ...
        val deviceListIndex = 0 // change this to display a specific device, -1 for all
    ...
    fun DeviceItem(device: Device) {
        ...
        val connectionState = device.viewModel.connectionStateFlow
            .collectAsState(SocketConnectionState.Disconnected(device.socketConnection.socketAddress))

        LaunchedEffect(Unit) {
            device.viewModel.ping()
        }

        KeyValueRow("Name:", device.name)
        KeyValueRow("Address:", device.socketConnection.socketAddress.address)
        KeyValueRow("State:", connectionState!!.value::class.simpleName ?: "N/A")
        Spacer(modifier = Modifier.height(8.dp))
        Button(
            enabled = (device.socketConnection.pingRequest == PingRequest.None && connectionState.value is SocketConnectionState.Connected)
